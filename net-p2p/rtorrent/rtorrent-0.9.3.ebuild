...
IUSE="color daemon debug ipv6 test vi-keys xmlrpc"
...
src_prepare() {
	...
	if use color; then
		epatch "${FILESDIR}"/${P}-canvas_color.patch
	fi

	if use vi-keys; then
		epatch "${FILESDIR}"/${P}-vi_keybindings.patch
	fi
...
pkg_postinst() {
	if use color; then
		einfo "Set colors using the options below in rtorrent.rc to color values 0-7:"
		einfo "done_fg_color, done_bg_color, active_fg_color, active_bg_color"
	fi
}
